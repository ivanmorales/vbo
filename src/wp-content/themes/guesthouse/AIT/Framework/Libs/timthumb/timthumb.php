...
class timthumb {
	...
	protected function getLocalImagePath($src){
		...
		if(file_exists ($this->docRoot . '/' . $src)) {
			...
			if(stripos($real, $this->docRoot) === 0 || stripos($real, $this->docRoot) === false){
				return $real;
			} else {
		...
		if($absolute && file_exists($absolute)){ //realpath does file_exists check, so can probably skip the exists check here
			...
			if(stripos($absolute, $this->docRoot) == 0 || stripos($absolute, $this->docRoot) == false){
				return $absolute;
			} else {
				$this->debug(1, "Security block: The file specified occurs outside the document root.");
				//and continue search
		...
		foreach ($sub_directories as $sub){
			...
			if(file_exists($base . $src)){
				...
				if(stripos($real, realpath($this->docRoot)) == 0 || stripos($real, realpath($this->docRoot)) == false){ 
					return $real;
				} else {
					$this->debug(1, "Security block: The file specified occurs outside the document root.");
					//And continue search
